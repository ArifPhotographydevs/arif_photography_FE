...
function ProposalCreate() {
  ...
  const [proposalData, setProposalData] = useState<ProposalData>({
    clientName: 'Sarah Johnson', // Pre-filled from lead
    shootType: 'Wedding', // Pre-filled from lead
    eventDate: '2024-03-15', // Pre-filled from lead
    venue: '',
    notes: '',
    services: [
      {
  ...
  const handleLogoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      setProposalData(prev => ({ ...prev, logo: file }));
  ...
  const calculateSubtotal = () => {
    const servicesTotal = proposalData.services.reduce((total, service) => 
      total + (service.quantity * service.unitPrice), 0
    );
    const addOnsTotal = proposalData.addOns
...
const handleSendProposal = async () => {
  ...
  const isStepValid = (step: number) => {
    switch (step) {
      ...
      case 2:
        return proposalData.services.every(service => 
          service.title && service.unitPrice > 0
        );
      case 3:
        return proposalData.termsTemplate !== '';
      case 4:
        return true;
      default:
        return false;
                ...
                {steps.map((step, index) => (
                  <div key={step.number} className="flex items-center">
                    ...
                    <span className={`ml-2 text-sm font-medium ${
                      currentStep >= step.number ? 'text-[#2D2D2D]' : 'text-gray-500'
                    }`}>
                      {step.title}
                    </span>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          readOnly
                          className="w-full pl-10 pr-3 py-2 bg-gray-100 border border-gray-200 rounded-lg text-[#2D2D2D] cursor-not-allowed"
                        />
                      </div>
                    </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        onClick={addService}
                        className="flex items-center px-3 py-2 bg-[#FF6B00] text-white rounded-lg font-medium hover:bg-[#e55a00] transition-colors duration-200"
                      >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          <div>
                            ...
                            <input
                              ...
                              className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-[#2D2D2D] placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#00BCEB] focus:border-[#00BCEB] transition-all duration-200"
                          ...
                          <div>
                            ...
                            <input
                              ...
                              className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-[#2D2D2D] focus:outline-none focus:ring-2 focus:ring-[#00BCEB] focus:border-[#00BCEB] transition-all duration-200"
                            />
                          </div>
                          ...
                          <div>
                            ...
                            <div className="relative">
                              ...
                              <input
                                ...
                                onChange={(e) => handleServiceChange(service.id, 'unitPrice', parseInt(e.target.value) || 0)}
                                className="w-full pl-10 pr-3 py-2 bg-white border border-gray-200 rounded-lg text-[#2D2D2D] focus:outline-none focus:ring-2 focus:ring-[#00BCEB] focus:border-[#00BCEB] transition-all duration-200"
                                placeholder="0"
                              />
                            </div>
                          </div>
                        ...
                        <div
                          ...
                          className={`p-3 border-2 rounded-lg cursor-pointer transition-all duration-200 ${
                            addOn.selected
                              ...
                              : 'border-gray-200 hover:border-[#00BCEB]/50'
                          }`}
                  ...
                  <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                    ...
                    <button
                      onClick={() => handleInputChange('gstEnabled', !proposalData.gstEnabled)}
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 ${
                        proposalData.gstEnabled ? 'bg-[#00BCEB]' : 'bg-gray-300'
                      }`}
                    >
                      <span
                        className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform duration-200 ${
                          proposalData.gstEnabled ? 'translate-x-6' : 'translate-x-1'
                        }`}
                      />
                    </button>
                  </div>
                  ...
                  <div className="bg-[#00BCEB]/5 rounded-lg p-4 space-y-2">
                    ...
                    {proposalData.gstEnabled && (
                      <div className="flex justify-between text-[#2D2D2D]">
                        <span>GST (18%):</span>
                        <span>₹{(calculateSubtotal() * 0.18).toLocaleString()}</span>
                      </div>
                    )}
                    <div className="flex justify-between text-lg font-semibold text-[#2D2D2D] pt-2 border-t border-[#00BCEB]/20">
                      <span>Total:</span>
                ...
                <div className="space-y-6">
                    ...
                    {!logoPreview ? (
                      ...
                      <div className="relative inline-block">
                        ...
                        <button
                          onClick={() => {
                            setLogoPreview(null);
                            setProposalData(prev => ({ ...prev, logo: null }));
                          }}
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <select
                        value={proposalData.termsTemplate}
                        onChange={(e) => handleInputChange('termsTemplate', e.target.value)}
                  ...
                  <div>
                    ...
                    <textarea
                      value={proposalData.footerNote}
                      onChange={(e) => handleInputChange('footerNote', e.target.value)}
                      rows={4}
                      className="w-full px-3 py-2 bg-[#F5F7FA] border border-gray-200 rounded-lg text-[#2D2D2D] placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#00BCEB] focus:border-[#00BCEB] transition-all duration-200 resize-none"
                      placeholder="Add any personalized message or condition..."
                    />
                  </div>
                </div>
              )}
                  ...
                  <div className="bg-gray-50 rounded-lg p-6 max-h-96 overflow-y-auto">
                    <div className="bg-white p-6 rounded-lg shadow-sm">
                      ...
                      <div className="mb-6">
                        ...
                        <p className="text-gray-600">{new Date(proposalData.eventDate).toLocaleDateString()}</p>
                      ...
                      {/* Add-ons */}
                      {proposalData.addOns.some(addOn => addOn.selected) && (
                        <div className="mb-6">
                            ...
                            {proposalData.addOns
                              ...
                              .map((addOn) => (
                                <div key={addOn.id} className="flex justify-between">
                                  <p className="text-[#2D2D2D]">{addOn.name}</p>
                                  <p className="font-medium text-[#2D2D2D]">₹{addOn.price.toLocaleString()}</p>
                                </div>
                              ))}
                          </div>
                        </div>
                      ...
                      <div className="border-t pt-4">
                        <div className="flex justify-between mb-2">
                          ...
                          <span className="text-[#2D2D2D]">₹{calculateSubtotal().toLocaleString()}</span>
                        </div>
                        {proposalData.gstEnabled && (
                          <div className="flex justify-between mb-2">
                            <span className="text-[#2D2D2D]">GST (18%):</span>
                            <span className="text-[#2D2D2D]">₹{(calculateSubtotal() * 0.18).toLocaleString()}</span>
                          </div>
                        )}
                ...
                {currentStep < 4 ? (
                  ...
                  <div className="flex items-center space-x-3">
                    <button
                      ...
                      className="px-4 py-2 bg-gray-100 text-[#2D2D2D] rounded-lg font-medium hover:bg-gray-200 transition-colors duration-200"
                    >
                      Go Back
                    </button>
                    <button
